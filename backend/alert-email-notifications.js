...
export function registerAlertEmailNotificationRoutes(app) {
  ...
  app.patch('/notification-preferences', requireUserAuth, async (req, res, next) => {
    ...
    try {
      const { rows } = await query(
        `INSERT INTO alert_notification_preferences (organization_id, user_id, email_alerts_enabled, updated_at)
         VALUES ($1, $2, $3, now())
         ON CONFLICT (organization_id, user_id) DO UPDATE SET
           email_alerts_enabled=EXCLUDED.email_alerts_enabled,
           updated_at=now()
         RETURNING email_alerts_enabled`,
        [req.user.organizationId, req.user.id, emailEnabled]
      );
      res.json({ notifications: publicPreferences(rows[0]) });
    } catch (error) {
      next(error);
...
async function reserveDelivery(organizationId, alert, userId) {
  const { rows } = await query(
    `INSERT INTO alert_email_deliveries (
       organization_id, alert_id, occurrence_started_at, user_id, status,
       attempt_count, last_attempt_at, last_error
     ) VALUES ($1, $2, $3, $4, 'sending', 1, now(), '')
     ON CONFLICT (organization_id, alert_id, occurrence_started_at, user_id) DO UPDATE SET
       status='sending',
     ...
     WHERE (alert_email_deliveries.status='failed'
       ...
       AND alert_email_deliveries.attempt_count < $5
       AND alert_email_deliveries.last_attempt_at < now() - ($6 || ' minutes')::interval
     RETURNING alert_id, occurrence_started_at`,
    [organizationId, alert.id, alert.occurrenceStartedAt, userId, MAX_DELIVERY_ATTEMPTS, String(DELIVERY_RETRY_MINUTES)]
  );
...
async function markDeliveries(organizationId, alerts, userId, status, error = '') {
  ...
  for (const alert of alerts) {
    await query(
      ...
      [status, String(error || '').slice(0, 500), organizationId, alert.id, alert.occurrenceStartedAt, userId]
    );
  }
}
  ...
  const [{ rows: organizationRows }, { rows: alertRows }, { rows: recipientRows }] = await Promise.all([
    ...
    query(
      `SELECT alert_id, first_detected_at, context
       ...
       WHERE organization_id=$1 AND managed=true AND active=true AND status='open'
