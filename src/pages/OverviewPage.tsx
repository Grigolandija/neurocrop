...
export function OverviewPage({ locations, blocks, activeBlock, mode, onBlock }: { locations: Location[]; blocks: Block[]; activeBlock: Block; mode: ViewMode; onBlock: (id: string) => void }) {
  ...
  return <>
    <section className="hero-panel">
      <div className="hero-head"><div><p className="eyebrow">Current growing conditions</p><h1>{activeBlock.cropProfile}</h1><p>{location?.name} · {activeBlock.name}</p></div><div className="score" data-state={score < 80 ? 'warning' : 'optimal'}><small>Block score</small><strong>{score}</strong><span>/ 100</span></div></div>
    ...
    <section className="next-step"><p className="eyebrow light">Recommended next step</p><h2>{primary ? `Tune ${primary.label.toLowerCase()}` : 'Keep the current setup'}</h2><p>{primary ? `The latest ${primary.label.toLowerCase()} reading is outside the configured target band.` : 'All installed growth metrics are inside their configured target bands.'}</p></section>
