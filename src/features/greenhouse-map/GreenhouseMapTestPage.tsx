    ...
    <div className="gh-workspace">
      <aside className="gh-left-panel">
        ...
        <section className="gh-panel-section gh-view-settings">
          ...
          <label className="gh-field wide"><span>{tr('Environment metric', 'Aplinkos rodiklis')}</span><select value={editor.map.heatmapSettings.metric} onChange={(event) => updateMetric(event.target.value as MetricKey)}>{(Object.keys(METRICS) as MetricKey[]).map((metric) => <option value={metric} key={metric}>{language === 'lt' ? METRICS[metric].labelLt : METRICS[metric].label}</option>)}</select></label>
          ...
          <div className="gh-field-row"><label className="gh-field"><span>{tr('Cell size', 'Celės dydis')} <em>m</em></span><NumericInput min=".1" max="5" step=".05" value={editor.map.heatmapSettings.cellSizeM} onCommit={(value) => editor.commit((map) => ({ ...map, heatmapSettings: { ...map.heatmapSettings, cellSizeM: value ?? map.heatmapSettings.cellSizeM } }))} /></label><label className="gh-field"><span>{tr('Nearest sensors', 'Artimiausi sensoriai')}</span><NumericInput min="3" max="5" step="1" value={editor.map.heatmapSettings.nearestSensorCount} onCommit={(value) => editor.commit((map) => { const nearestSensorCount = Math.max(3, Math.min(5, Math.round(value ?? map.heatmapSettings.nearestSensorCount))); return { ...map, heatmapSettings: { ...map.heatmapSettings, nearestSensorCount, minimumSensorCount: Math.min(map.heatmapSettings.minimumSensorCount, nearestSensorCount) } } })} /></label></div>
          <div className="gh-field-row"><label className="gh-field"><span>{tr('Minimum sensors per cell', 'Mažiausiai sensorių celei')}</span><NumericInput min="2" max="5" step="1" value={editor.map.heatmapSettings.minimumSensorCount} onCommit={(value) => editor.commit((map) => ({ ...map, heatmapSettings: { ...map.heatmapSettings, minimumSensorCount: Math.max(2, Math.min(map.heatmapSettings.nearestSensorCount, Math.round(value ?? map.heatmapSettings.minimumSensorCount))) } }))} /></label><label className="gh-field"><span>{tr('Influence radius', 'Įtakos spindulys')} <em>m</em></span><NumericInput min=".1" max="10000" step=".5" value={editor.map.heatmapSettings.maxInfluenceDistanceM} onCommit={(value) => editor.commit((map) => ({ ...map, heatmapSettings: { ...map.heatmapSettings, maxInfluenceDistanceM: value ?? map.heatmapSettings.maxInfluenceDistanceM } }))} /></label></div>
          <label className="gh-field wide"><span>{tr('Maximum reading age', 'Didžiausias duomenų amžius')} <em>min</em></span><NumericInput min="1" max="10080" step="1" value={editor.map.heatmapSettings.maxReadingAgeMinutes} onCommit={(value) => editor.commit((map) => ({ ...map, heatmapSettings: { ...map.heatmapSettings, maxReadingAgeMinutes: value ?? map.heatmapSettings.maxReadingAgeMinutes } }))} /></label>
