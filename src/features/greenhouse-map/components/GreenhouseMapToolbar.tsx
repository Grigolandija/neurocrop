...
export default function GreenhouseMapToolbar(props: Props) {
  ...
  return <header className="gh-toolbar">
    ...
    <nav className="gh-modes" aria-label={tr('Map mode', 'Žemėlapio režimas')}>
      {([
        ['layout', 'fa-pen-ruler', tr('Plan', 'Planas'), tr('Arrange node positions and greenhouse equipment.', 'Išdėstykite mazgus ir šiltnamio įrangą.')],
    ...
    <div className="gh-toolbar-actions">
      {props.editing ? <>
        <button className={props.snap ? 'active' : ''} onClick={() => props.onSnap(!props.snap)} title={tr('Snap objects to the plan grid', 'Lygiuoti objektus pagal plano tinklelį')}><i className="fa-solid fa-magnet" /> {tr('Snap', 'Lygiavimas')}</button>
        <span />
