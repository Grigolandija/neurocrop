...
export default function LayersPanel({ layers, onChange }: { layers: MapLayer[]; onChange: (layers: MapLayer[]) => void }) {
  ...
  return <section className="gh-panel-section gh-layers">
    <header><div><small>RENDER STACK</small><h2>Layers</h2></div><span>{layers.filter((layer) => layer.visible).length}/{layers.length}</span></header>
