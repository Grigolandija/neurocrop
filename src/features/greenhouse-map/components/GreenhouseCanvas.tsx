  ...
  const heatmapLegend = mode === 'environment' ? <div className={`gh-heatmap-legend ${compactLegend ? 'gh-heatmap-legend-compact' : ''}`}>
    ...
    {heatmap ? <>
      ...
      <div className="gh-legend-meta">
        {!compactLegend ? <em className="gh-color-mode-note" title={colorMode === 'contrast' ? tr('Relative colours: current frame minimum to maximum.', 'Santykinės spalvos: nuo šio kadro minimumo iki maksimumo.') : tr('Stable colours: the same value always keeps the same colour.', 'Stabilios spalvos: ta pati reikšmė visada turi tą pačią spalvą.')}><i className={`fa-solid ${colorMode === 'contrast' ? 'fa-circle-half-stroke' : 'fa-temperature-half'}`} /> {colorMode === 'contrast' ? tr('Relative colours: current frame minimum to maximum.', 'Santykinės spalvos: nuo šio kadro minimumo iki maksimumo.') : tr('Stable colours: the same value always keeps the same colour.', 'Stabilios spalvos: ta pati reikšmė visada turi tą pačią spalvą.')}</em> : null}
        ...
        {!compactLegend ? <p><b>{heatmap.count} {tr('sensor sources', 'sensorių šaltiniai')}</b><span>{Math.round(heatmap.grid.dataCellCount / Math.max(1, heatmap.grid.width * heatmap.grid.height) * 100)}% {tr('raster coverage', 'rasterio padengimas')} · {heatmap.grid.cellWidthM.toFixed(2)} m</span><span>{tr('Rendered', 'Atvaizduota')} {heatmap.calculatedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></p> : null}
        {!compactLegend && map.heatmapSettings.metric === 'soil-ec' && soilEcDepthCm !== undefined ? <em><i className="fa-solid fa-layer-group" /> {soilEcDepthCm} cm {tr('depth', 'gylis')}</em> : null}
        {!compactLegend && map.heatmapSettings.metric === 'soil-ec' && soilEcDepths.length > 1 ? <em><i className="fa-solid fa-crosshairs" /> {tr('Click the map for a vertical profile.', 'Paspauskite žemėlapį vertikaliam profiliui.')}</em> : null}
        {!compactLegend && !heatmap.grid.dataCellCount ? <em>{tr('No cells meet the distance, freshness and minimum-sensor rules.', 'Nė viena celė neatitinka atstumo, šviežumo ir minimalaus sensorių skaičiaus taisyklių.')}</em> : null}
        {!compactLegend && heatmap.count >= MIN_CONTOUR_SENSOR_COUNT && heatmap.count < 4 ? <em>{tr(`Contour spacing reflects limited coverage from ${heatmap.count} nodes.`, `Izolinijų žingsnis atspindi ribotą ${heatmap.count} mazgų padengimą.`)}</em> : !compactLegend && heatmap.count < MIN_CONTOUR_SENSOR_COUNT ? <em>{tr('Contour lines need at least two valid nodes.', 'Izolinijoms reikia bent dviejų tinkamų mazgų.')}</em> : null}
        {!compactLegend ? <em><i className="fa-solid fa-circle-info" /> {tr('Estimated between sensor locations.', 'Įvertinta tarp sensorių vietų.')}</em> : null}
      </div>
