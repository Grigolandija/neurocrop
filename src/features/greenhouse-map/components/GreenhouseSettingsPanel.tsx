...
export default function GreenhouseSettingsPanel({ map, language = 'en', onChange }: Props) {
  ...
  const updateDimensions = (field: 'widthM' | 'lengthM' | 'heightM', value: number | undefined) => onChange({ ...map, dimensions: { ...map.dimensions, [field]: value } })
  return <section className="gh-panel-section">
    ...
    <div className="gh-field-row">
      ...
      <label className="gh-field"><span>{tr('Orientation', 'Orientacija')} <em>°</em></span><NumericInput value={map.orientationDeg} onCommit={(value) => onChange({ ...map, orientationDeg: value ?? map.orientationDeg })} /></label>
    </div>
    <div className="gh-field-row">
      ...
      <label className="gh-field"><span>{tr('Wall', 'Siena')} <em>m</em></span><NumericInput min="0.01" step="0.01" value={map.wallThicknessM} onCommit={(value) => onChange({ ...map, wallThicknessM: value ?? map.wallThicknessM })} /></label>
    </div>
  </section>
}
