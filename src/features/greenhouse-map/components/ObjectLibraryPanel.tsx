...
const ltLabels: Partial<Record<ObjectType, string>> = {
...
export default function ObjectLibraryPanel({ language = 'en', onAdd }: { language?: 'en' | 'lt'; onAdd: (type: ObjectType) => void }) {
  ...
  const entries = OBJECT_LIBRARY.filter((entry) => `${entry.label} ${ltLabels[entry.type] || ''}`.toLowerCase().includes(query.toLowerCase()))
  return <section className="gh-panel-section gh-library">
    ...
    <div className="gh-library-grid">
      ...
      ><i className={`fa-solid ${entry.icon}`} /><span>{language === 'lt' ? ltLabels[entry.type] || entry.label : entry.label}</span><b>+</b></button>)}
    </div>
  </section>
}
