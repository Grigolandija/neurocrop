...
export default function CropProfilesWorkspace() {
  ...
  if (status === 'loading') return <div className="crop-profile-standalone nc-profile-state" data-react-crop-profiles aria-busy="true"><i className="fa-solid fa-spinner fa-spin" /><h2>{tx("Loading crop profiles…")}</h2></div>
  ...
  return <div className="crop-profile-standalone" data-react-crop-profiles><section className="crop-profiles-page"><form className="crop-profile-editor" onSubmit={saveProfile}>
    <nav className="profile-detail-breadcrumb" aria-label="Breadcrumb"><button type="button" onClick={() => { setSelectedId(''); setDraft(null); setError('') }}>{tx("Crop profiles")}</button><i className="fa-solid fa-chevron-right" /><span>{draft.name}</span></nav>
    <header className="profile-detail-header"><div className="profile-detail-heading"><p className="profile-detail-eyebrow">{draft.heroName} <span>·</span> {draft.stage || 'Custom program'}</p><h2>{draft.name}</h2><span>{tx("Used by")} {usage[draft.id] || 0} {tx("section")}{usage[draft.id] === 1 ? '' : 's'}{tx(". Changes affect future scoring and alerts after saving.")}</span></div><div className="profile-detail-actions"><button type="button" className="settings-secondary-button" onClick={() => setDuplicateState({ source: draft, id: `${draft.id}-copy`, name: `${draft.name} copy` })}><i className="fa-regular fa-copy" />{tx("Duplicate")}</button>{draft.id === 'default' ? <span className="default-profile-header-badge"><i className="fa-solid fa-lock" />{tx("Protected")}</span> : <button type="button" className="settings-secondary-button crop-profile-header-delete" onClick={() => setDeleteState({ profile: draft, replacementId: '' })}><i className="fa-regular fa-trash-can" />{tx("Delete")}</button>}<button type="button" className="crop-profile-discard-button" disabled={!dirty || busy} onClick={() => setDraft(clone(selected))}>{tx("Discard")}</button><button type="submit" className="settings-primary-button" disabled={!dirty || busy}><i className={`fa-solid ${busy ? 'fa-spinner fa-spin' : 'fa-floppy-disk'}`} />{busy ?tx("Saving…") :tx("Save changes")}</button></div></header>
...
function LibraryDialog({ busy, error, onClose, onUse }: { busy: boolean; error: string; onClose: () => void; onUse: (template: CropProfileLibraryTemplate) => void }) {
  ...
  return <div className="profile-create-backdrop profile-library-backdrop"><section className="crop-profile-library" role="dialog" aria-modal="true" aria-labelledby="cropProfileLibraryTitle"><header><div><p>{tx("NeuroCrop defaults")} · v{cropProfileLibraryVersion}</p><h3 id="cropProfileLibraryTitle">{tx("Crop profile library")}</h3><span>{tx("Choose a crop and growth stage. NeuroCrop creates an independent workspace copy that you can review and adapt.")}</span></div><button type="button" onClick={onClose} aria-label={tx("Close library")}><i className="fa-solid fa-xmark" /></button></header>
    ...
    <div className="profile-library-list">{templates.map((template) => {
      ...
      return <article className="profile-library-item" key={template.profile_id}><span className="crop-monogram">{template.phase.order}</span><div><small>{duration} · {template.confidence} confidence</small><h4>{template.phase.name_en}</h4><div className="profile-library-metrics">{enabled.slice(0, 5).map((parameter) => <span key={parameter.code}>{getMetricDefinition(parameter.metricId)?.label || parameter.labelLt} <b>{parameter.min}–{parameter.max} {parameter.unit}</b></span>)}{enabled.length > 5 ? <span>+{enabled.length - 5} parameters</span> : null}</div></div><button type="button" className="settings-primary-button" disabled={busy} onClick={() => void onUse(template)}>{busy ? tx("Creating…") : tx("Use template")}</button></article>
    })}</div>
...
function CreateDialog({ state, profiles, busy, error, onChange, onClose, onSubmit }: { state: CreateState; profiles: Profile[]; busy: boolean; error: string; onChange: (state: CreateState) => void; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  return <div className="profile-create-backdrop"><section className="crop-profile-create-drawer" role="dialog" aria-modal="true" aria-labelledby="createCropProfileTitle"><header><div><p>{tx("New agronomic program")}</p><h3 id="createCropProfileTitle">{tx("Create crop profile")}</h3><span>{tx("Start from a complete target set, then adjust it for this crop stage.")}</span></div><button type="button" onClick={onClose}><i className="fa-solid fa-xmark" /></button></header><form onSubmit={onSubmit}><label className="profile-create-field profile-create-field-wide"><span>{tx("Profile name")}</span><small>{tx("A clear name shown when assigning this profile to Sections.")}</small><input autoFocus value={state.name} onChange={(event) => onChange({ ...state, name: event.target.value })} required /></label><label className="profile-create-field"><span>{tx("Crop")}</span><input value={state.heroName} onChange={(event) => onChange({ ...state, heroName: event.target.value })} required /></label><label className="profile-create-field"><span>{tx("Growth stage")}</span><input value={state.stage} onChange={(event) => onChange({ ...state, stage: event.target.value })} /></label><label className="profile-create-field profile-create-field-wide profile-create-source"><span>{tx("Copy targets from")}</span><small>{tx("The new profile starts with these optimal ranges.")}</small><span className="profile-create-select-wrap"><select value={state.sourceId} onChange={(event) => onChange({ ...state, sourceId: event.target.value })}>{profiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.name}</option>)}</select><i className="fa-solid fa-chevron-down" /></span></label>{error ? <p className="nc-profile-modal-error" role="alert">{error}</p> : null}<footer><button type="button" className="settings-secondary-button" onClick={onClose}>{tx("Cancel")}</button><button type="submit" className="settings-primary-button" disabled={busy}>{busy ? 'Creating…' :tx("Create profile")}</button></footer></form></section></div>
}
...
function DuplicateDialog({ state, busy, error, onChange, onClose, onSubmit }: { state: DuplicateState; busy: boolean; error: string; onChange: (state: DuplicateState) => void; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  return <div className="profile-create-backdrop"><section className="crop-profile-create-drawer" role="dialog" aria-modal="true"><header><div><p>{tx("Copy agronomic program")}</p><h3>{tx("Duplicate")} {state.source.name}</h3><span>{tx("Create an independent copy with the same targets.")}</span></div><button type="button" onClick={onClose}><i className="fa-solid fa-xmark" /></button></header><form onSubmit={onSubmit}><label className="profile-create-field profile-create-field-wide"><span>{tx("Profile name")}</span><input autoFocus value={state.name} onChange={(event) => onChange({ ...state, name: event.target.value, id: slug(event.target.value) })} required /></label><label className="profile-create-field profile-create-field-wide"><span>{tx("Profile ID")}</span><input value={state.id} pattern="[a-z0-9-]+" onChange={(event) => onChange({ ...state, id: slug(event.target.value) })} required /></label>{error ? <p className="nc-profile-modal-error" role="alert">{error}</p> : null}<footer><button type="button" className="settings-secondary-button" onClick={onClose}>{tx("Cancel")}</button><button type="submit" className="settings-primary-button" disabled={busy}>{busy ? 'Duplicating…' :tx("Duplicate")}</button></footer></form></section></div>
}
...
function DeleteDialog({ state, profiles, assigned, busy, error, onChange, onClose, onSubmit }: { state: DeleteState; profiles: Profile[]; assigned: number; busy: boolean; error: string; onChange: (state: DeleteState) => void; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  ...
  return <div className="profile-create-backdrop profile-delete-backdrop"><section className="crop-profile-delete-dialog" role="dialog" aria-modal="true"><header><span className="profile-delete-icon"><i className="fa-solid fa-trash" /></span><div><p>{tx("Permanent action")}</p><h3>{tx("Delete")} {state.profile.name}?</h3></div><button type="button" onClick={onClose}><i className="fa-solid fa-xmark" /></button></header><form onSubmit={onSubmit}>{assigned ? <><div className="profile-delete-impact"><strong>{assigned} {tx("assigned section")}{assigned === 1 ? '' : 's'}</strong><span>{tx("Their measurement history will be kept. Choose the profile that should control future scoring and alerts.")}</span></div><label><span>{tx("Replacement crop profile")}</span><select value={state.replacementId} required onChange={(event) => onChange({ ...state, replacementId: event.target.value })}><option value="">{tx("Select replacement profile")}</option>{replacements.map((profile) => <option value={profile.id} key={profile.id}>{profile.name} · {profile.stage}</option>)}</select></label></> : <div className="profile-delete-impact"><strong>{tx("This profile is not assigned to any section.")}</strong><span>{tx("Deleting it will not remove measurements, sections, or nodes.")}</span></div>}{error ? <p className="nc-profile-modal-error" role="alert">{error}</p> : null}<footer><button type="button" className="settings-secondary-button" onClick={onClose}>{tx("Cancel")}</button><button type="submit" className="crop-profile-delete-confirm" disabled={busy || (assigned > 0 && !state.replacementId)}>{tx("Delete profile")}</button></footer></form></section></div>
}
