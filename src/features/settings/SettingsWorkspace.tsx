...
import ClerkSecuritySettings from './ClerkSecuritySettings'
...
export default function SettingsWorkspace({ initialSection = 'workspace' }: { initialSection?: SettingsSection }) {
  ...
  return <main className="nc-settings-page" aria-busy={loading}>
    <header className="nc-settings-page-head"><div><p>{tx("Administration")}</p><h1>{tx("Settings")}</h1><span>{tx("Workspace configuration, access, delivery, and account security in one accountable place.")}</span></div><div className="nc-settings-save-state" data-dirty={dirty}><i /><span><strong>{dirty ?tx("Unsaved changes") :tx("All settings saved")}</strong><small>{dirty ?tx("Review before leaving") : user?.organizationName ||tx("Workspace configuration")}</small></span></div></header>
    ...
    <section className="nc-settings-center">
      <aside className="nc-settings-nav" aria-label={tx("Settings sections")}>
        <div className="nc-settings-identity"><span>{initials(user?.organizationName || 'NeuroCrop')}</span><div><strong>{user?.organizationName ||tx("Loading workspace")}</strong><small>{user?.role ? `${user.role} access` :tx("Authenticated workspace")}</small></div></div>
        <p>{tx("Workspace")}</p>
        {([
      ...
      <div className="nc-settings-main">
        <header className="nc-settings-panel-head"><div><p>{meta.eyebrow}</p><h2>{meta.title}</h2><span>{meta.description}</span></div>{section === 'audit' ? <button className="nc-settings-button secondary" onClick={exportAudit}><i className="fa-solid fa-download" />{tx("Export")}</button> : null}</header>

        {section === 'workspace' ? <div className="nc-settings-flow"><section className="nc-settings-card"><header><div><h3>{tx("Workspace identity")}</h3><p>{tx("Shared name used throughout NeuroCrop and team invitations.")}</p></div><Status tone="neutral">{tx("Workspace-wide")}</Status></header><div className="nc-settings-fields"><label><span>{tx("Organization name")}</span><input value={draft.organizationName} disabled={!canManageTeam} onChange={(event) => setDraft({ ...draft, organizationName: event.target.value })} /></label><label><span>{tx("Signed-in account")}</span><input value={user?.email || ''} disabled /></label></div>{!canManageTeam ? <p className="nc-settings-helper">{tx("Only an owner or administrator can rename this workspace.")}</p> : null}</section><section className="nc-settings-card"><header><div><h3>{tx("Your display preferences")}</h3><p>{tx("Stored for this browser. Other team members keep their own preferences.")}</p></div><Status tone="info">{tx("Personal")}</Status></header><div className="nc-settings-fields three"><label><span>{tx("Language")}</span><select value={draft.locale} onChange={(event) => setDraft({ ...draft, locale: event.target.value })}><option value="en-GB">{tx("English")}</option><option value="lt-LT">{tx("Lithuanian")}</option></select></label><label><span>{tx("Time zone")}</span><select value={draft.timezone} onChange={(event) => setDraft({ ...draft, timezone: event.target.value })}><option>Europe/Vilnius</option><option>Europe/Riga</option><option>Europe/Warsaw</option></select></label><label><span>{tx("Time format")}</span><select value={draft.timeFormat} onChange={(event) => setDraft({ ...draft, timeFormat: event.target.value })}><option value="24h">{tx("24 hour")}</option><option value="12h">{tx("12 hour")}</option></select></label><label><span>{tx("Temperature")}</span><select value={draft.temperatureUnit} onChange={(event) => setDraft({ ...draft, temperatureUnit: event.target.value })}><option value="C">{tx("Celsius")}</option><option value="F">{tx("Fahrenheit")}</option></select></label></div></section><section className="nc-settings-note"><i className="fa-solid fa-seedling" /><div><strong>{tx("Agronomic targets live in Crop profiles")}</strong><p>{tx("Temperature, humidity, VPD, lighting, and root-zone limits are intentionally not duplicated here.")}</p></div><button onClick={() => navigate('/crop-profiles')}>{tx("Open crop profiles")} <i className="fa-solid fa-arrow-right" /></button></section></div> : null}
        ...
        ] as Array<[keyof Preferences, string, string, string]>).map(([key, icon, label, note]) => <label key={key}><i className={`fa-solid ${icon}`} /><span><strong>{label}</strong><small>{note}</small></span><input type="checkbox" checked={Boolean(draft[key])} onChange={() => setDraft({ ...draft, [key]: !draft[key] })} /><i className="nc-settings-switch" /></label>)}</div></section><section className="nc-settings-card"><header><div><h3>{tx("Escalation behavior")}</h3><p>{tx("Controls notification timing, not crop-profile target limits.")}</p></div></header><div className="nc-settings-fields three"><label><span>{tx("Warning persistence")}</span><select value={draft.warningAfterMinutes} onChange={(event) => setDraft({ ...draft, warningAfterMinutes: Number(event.target.value) })}><option value="10">{tx("10 minutes")}</option><option value="15">{tx("15 minutes")}</option><option value="30">{tx("30 minutes")}</option></select></label><label><span>{tx("Quiet hours start")}</span><input type="time" value={draft.quietStart} onChange={(event) => setDraft({ ...draft, quietStart: event.target.value })} /></label><label><span>{tx("Quiet hours end")}</span><input type="time" value={draft.quietEnd} onChange={(event) => setDraft({ ...draft, quietEnd: event.target.value })} /></label></div><label className="nc-settings-inline-toggle"><span><strong>{tx("Critical alerts override quiet hours")}</strong><small>{tx("Critical conditions should be delivered immediately.")}</small></span><input type="checkbox" checked={draft.criticalOverride} onChange={() => setDraft({ ...draft, criticalOverride: !draft.criticalOverride })} /><i className="nc-settings-switch" /></label></section></div> : null}

        {section === 'security'
          ? clerkConfigured
            ? <ClerkSecuritySettings />
            : <div className="nc-settings-flow"><div className="nc-settings-security-grid"><form className="nc-settings-card" onSubmit={changePassword}><header><div><h3>{tx("Change password")}</h3><p>{tx("At least 12 characters. Other sessions will be revoked.")}</p></div></header><div className="nc-settings-fields single"><label><span>{tx("Current password")}</span><input type="password" autoComplete="current-password" maxLength={1024} required value={passwords.current} onChange={(event) => setPasswords({ ...passwords, current: event.target.value })} /></label><label><span>{tx("New password")}</span><input type="password" autoComplete="new-password" minLength={12} maxLength={1024} required value={passwords.next} onChange={(event) => setPasswords({ ...passwords, next: event.target.value })} /></label><label><span>{tx("Confirm new password")}</span><input type="password" autoComplete="new-password" minLength={12} maxLength={1024} required value={passwords.confirm} onChange={(event) => setPasswords({ ...passwords, confirm: event.target.value })} /></label></div><button className="nc-settings-button primary" disabled={busyKey === 'password'}>{busyKey === 'password' ?tx("Updating…") :tx("Update password")}</button></form><section className="nc-settings-card"><header><div><h3>{tx("Active sessions")}</h3><p>{tx("Revoke any session you no longer recognize.")}</p></div></header>{sessionsAvailable ? <div className="nc-settings-sessions">{sessions.map((session) => <article key={session.id}><i className="fa-solid fa-display" /><div><strong>{session.current ?tx("Current browser session") :tx("Authenticated session")}</strong><small>{tx("Last active")} {formatDate(session.lastSeenAt || session.createdAt)}</small></div>{session.current ? <Status tone="success">{tx("Current")}</Status> : <button disabled={busyKey === `session-${session.id}`} onClick={() => void revokeSession(session)}>{tx("Sign out")}</button>}</article>)}</div> : <div className="nc-settings-empty"><i className="fa-solid fa-server" /><strong>{tx("Session management is being deployed")}</strong><span>{tx("Password changes still work. The signed-in device list will appear after the backend update.")}</span></div>}</section></div></div>
          : null}

        {section === 'audit' ? <div className="nc-settings-flow"><div className="nc-settings-audit-toolbar"><div>{(['all', 'access', 'configuration', 'security'] as const).map((filter) => <button className={auditFilter === filter ? 'active' : ''} key={filter} onClick={() => setAuditFilter(filter)}>{filter === 'all' ?tx("All activity") : filter}</button>)}</div><label><i className="fa-solid fa-magnifying-glass" /><input value={auditQuery} onChange={(event) => setAuditQuery(event.target.value)} placeholder={tx("Search actor or change")} /></label></div><section className="nc-settings-audit">{filteredAudit.map((event) => <article key={event.id}><time>{formatDate(event.time)}</time><span><i className={`fa-solid ${event.category === 'access' ? 'fa-user-shield' : event.category === 'security' ? 'fa-lock' : 'fa-sliders'}`} /></span><div><strong>{event.action}</strong><p>{event.detail}</p></div><small>{event.actor}</small></article>)}{filteredAudit.length === 0 ? <div className="nc-settings-empty"><i className="fa-solid fa-clock-rotate-left" /><strong>{tx("No matching settings activity")}</strong><span>{tx("Actions performed from this browser will appear here.")}</span></div> : null}</section><p className="nc-settings-helper">{tx("This is a browser-local settings activity record, not the future organization-wide backend audit log.")}</p></div> : null}
      </div>
    </section>
    {dirty ? <div className="nc-settings-save-dock" role="status"><div><i /><span><strong>{tx("Unsaved settings")}</strong><small>{tx("Your changes are not active yet.")}</small></span></div><button onClick={() => setDraft(saved)}>{tx("Discard")}</button><button className="primary" disabled={busyKey === 'save'} onClick={() => void savePreferences()}><i className="fa-solid fa-floppy-disk" />{busyKey === 'save' ?tx("Saving…") :tx("Save changes")}</button></div> : null}
  </main>
}
