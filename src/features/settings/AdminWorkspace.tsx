...
type PlatformUser = {
  ...
  organizations: Array<{ id: string; name: string; status: string; role: string }>
...
export default function AdminWorkspace() {
  ...
  const [userToMove, setUserToMove] = useState<PlatformUser | null>(null)
  const [moveOrganizationId, setMoveOrganizationId] = useState('')
  const [moveRole, setMoveRole] = useState('grower')
  ...
  function moveTargets(user: PlatformUser) {
    const active = organizations.filter((item) => item.status !== 'archived')
    if ((user.organizations || []).length !== 1) return active
    return active.filter((item) => item.id !== user.organizations[0].id)
  }

  function openMoveUser(user: PlatformUser) {
    const currentOrganizationIds = new Set((user.organizations || []).map((item) => item.id))
    const targets = moveTargets(user)
    const target = targets.find((item) => !currentOrganizationIds.has(item.id)) || targets[0]
    setUserToMove(user)
    setMoveOrganizationId(target?.id || '')
    setMoveRole(user.organizations?.find((item) => item.id === target?.id)?.role || user.organizations?.[0]?.role || 'grower')
    setFeedback(null)
  }

  async function moveUser(event: FormEvent) {
    event.preventDefault()
    if (!userToMove || !moveOrganizationId) return
    const target = organizations.find((item) => item.id === moveOrganizationId)
    setBusyKey(`move-user-${userToMove.id}`)
    setFeedback(null)
    try {
      await neurocropApi.movePlatformUser(userToMove.id, { organizationId: moveOrganizationId, role: moveRole })
      setUserToMove(null)
      setFeedback({ tone: 'success', text: `${userToMove.email} moved to ${target?.name || 'the selected organization'}.` })
      await load()
    } catch (reason) {
      setFeedback({ tone: 'warning', text: errorMessage(reason) })
    } finally {
  ...
  return <main className="nc-settings-page nc-admin-page" aria-busy={loading}>
    ...
    {userToMove ? <ModalPortal><div className="nc-admin-modal-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busyKey) setUserToMove(null) }}><section className="nc-admin-diagnostics nc-admin-move-user-dialog" role="dialog" aria-modal="true" aria-labelledby="adminMoveUserTitle"><header><div><p>{tx("Organization access")}</p><h2 id="adminMoveUserTitle">{tx("Move user")}</h2><span>{userToMove.name || userToMove.email} · {userToMove.email}</span></div><button disabled={Boolean(busyKey)} onClick={() => setUserToMove(null)} aria-label={tx("Close")}><i className="fa-solid fa-xmark" /></button></header><form onSubmit={moveUser}><div className="nc-admin-move-warning"><i className="fa-solid fa-right-left" /><div><strong>{tx("Replace organization access")}</strong><span>{tx("The user will lose access to every current organization. Empty personal organizations will be removed automatically; organizations with data require another owner.")}</span></div></div><label><span>{tx("Target organization")}</span><select required value={moveOrganizationId} onChange={(event) => setMoveOrganizationId(event.target.value)}>{moveTargets(userToMove).map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></label><label><span>{tx("Role in target organization")}</span><select value={moveRole} onChange={(event) => setMoveRole(event.target.value)}><option value="owner">{tx("Owner")}</option><option value="admin">{tx("Admin")}</option><option value="grower">{tx("Grower")}</option><option value="technician">{tx("Technician")}</option><option value="viewer">{tx("Viewer")}</option></select></label><footer><button type="button" className="secondary" disabled={Boolean(busyKey)} onClick={() => setUserToMove(null)}>{tx("Cancel")}</button><button type="submit" disabled={!moveOrganizationId || Boolean(busyKey)}><i className={`fa-solid ${busyKey ? 'fa-spinner fa-spin' : 'fa-right-left'}`} />{tx("Move user")}</button></footer></form></section></div></ModalPortal> : null}
