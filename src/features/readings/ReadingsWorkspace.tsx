...
export default function ReadingsWorkspace() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const [dashboardPayload, profilesPayload] = await Promise.all([
          neurocropApi.getDashboard(), neurocropApi.getCropProfiles().catch(() => ({ profiles: [] })),
        ...
        const flatSections = asArray(dashboardPayload?.sites).flatMap((site) => asArray(site.zones).map((zone) => ({ site, zone })))
        ...
        const nextSections = flatSections.map(({ site, zone }, index): SectionReading => {
          ...
          const profileId = String(zone.profile?.id || zone.profile || zone.profileId || 'default')
  ...
  const areas = useMemo(() => Array.from(new Map(sections.map((section) => [section.areaId, section.areaName]))), [sections])
  ...
  return <main className="nc-readings-workspace" aria-busy={status === 'loading'}>
    ...
    <section className="nc-readings-matrix" aria-labelledby="nc-readings-title">
      ...
      <div className="nc-readings-controls"><label><span>Area</span><select value={areaFilter} onChange={(event) => setAreaFilter(event.target.value)}><option value="all">All areas</option>{areas.map(([id, name]) => <option value={id} key={id}>{name}</option>)}</select></label><button type="button" className={attentionOnly ? 'active' : ''} onClick={() => setAttentionOnly(!attentionOnly)}><i className="fa-solid fa-filter" />Needs attention</button><label className="nc-sort"><span>Sort</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value)}><option value="severity">Most outside target</option><option value="freshest">Freshest data</option><option value="oldest">Oldest data</option><option value="area">Area</option><option value="section">Section</option></select></label></div>
    ...
    <section className="nc-signal-lens"><button type="button" onClick={() => setLensOpen(!lensOpen)} aria-expanded={lensOpen}><span><i className="fa-solid fa-chart-simple" /><span><strong>Profile-normalized signal lens</strong><small>Compare one parameter across different crop targets when you need deeper analysis.</small></span></span><i className={`fa-solid fa-chevron-${lensOpen ? 'up' : 'down'}`} /></button>{lensOpen ? <div className="nc-signal-lens-content"><header><h2>{lensMetric.label} across sections</h2><select value={lensMetricKey} onChange={(event) => setLensMetricKey(event.target.value)}>{metrics.map((metric) => <option value={metric.key} key={metric.key}>{metric.label}</option>)}</select></header>{visibleSections.map((section) => { const value = getValue(section, lensMetric); const range = getRange(profiles.get(section.profileId), lensMetric); const width = range ? Math.max(.001, range[1] - range[0]) : 1; const position = range && value !== null ? Math.max(4, Math.min(96, 50 + ((value - (range[0] + range[1]) / 2) / width) * 32)) : 4; return <div className="nc-distribution-row" key={section.id}><span><strong>{section.name}</strong><small>{section.areaName} · {section.profileName}</small></span><div><i style={{ left: `${position}%` }} data-tone={getTone(section, lensMetric, profiles.get(section.profileId))} /></div><strong>{formatValue(value, lensMetric)} <small>{lensMetric.unit}</small></strong><button onClick={() => togglePin(section.id)} disabled={!pinned.includes(section.id) && pinned.length >= 3}><i className="fa-solid fa-thumbtack" /></button></div>})}</div> : null}</section>
