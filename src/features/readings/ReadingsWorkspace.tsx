          ...
          {status === 'loading' && !sections.length ? Array.from({ length: 4 }, (_, index) => <div className="nc-reading-skeleton" key={index} />) : visibleSections.map((section) => {
            ...
            return <Fragment key={section.id}>
              <div className={`nc-readings-row nc-section-summary-row ${expanded ? 'expanded' : ''}`} style={matrixStyle}>
                <div className="nc-reading-section"><span data-state={normalizedDeviation(section, visibleMetrics, profiles) > 0 ? 'watch' : 'good'} /><button type="button" onClick={() => toggleSectionNodes(section.id)} title={`${section.name} · ${section.areaName} · ${section.profileName}`} aria-expanded={expanded} aria-controls={`nc-section-nodes-${section.id}`}><span><strong>{section.name}</strong><small>{tx("Section average")}</small></span></button><button type="button" className={pinned.includes(section.id) ? 'pinned' : ''} disabled={!pinned.includes(section.id) && pinned.length >= 3} onClick={() => togglePin(section.id)} aria-pressed={pinned.includes(section.id)} aria-label={`${pinned.includes(section.id) ? 'Unpin' : 'Pin'} ${section.name}`}><i className="fa-solid fa-thumbtack" /></button></div>
