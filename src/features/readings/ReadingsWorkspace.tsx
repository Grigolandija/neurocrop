...
export default function ReadingsWorkspace() {
  ...
  return <main className="nc-readings-workspace" aria-busy={status === 'loading'}>
    ...
    <section className="nc-readings-matrix" aria-labelledby="nc-readings-title">
      ...
      <div className="nc-readings-controls"><div className="nc-area-tabs" role="tablist" aria-label="Filter by Area">{workspaceView === 'table' ? <button type="button" role="tab" aria-selected={areaFilter === 'all'} className={areaFilter === 'all' ? 'active' : ''} onClick={() => setAreaFilter('all')}>All areas <b>{sections.length}</b></button> : null}{areaOptions.map(([id, name]) => <button type="button" role="tab" aria-selected={activeAreaFilter === id} className={activeAreaFilter === id ? 'active' : ''} onClick={() => setAreaFilter(id)} key={id}>{name} <b>{sections.filter((section) => section.areaId === id).length}</b></button>)}</div>{workspaceView === 'table' ? <div className="nc-readings-control-actions"><button type="button" className={attentionOnly ? 'active' : ''} onClick={() => setAttentionOnly(!attentionOnly)}><i className="fa-solid fa-filter" />Needs attention</button><label className="nc-sort"><span>Sort</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value)}><option value="severity">Most outside target</option><option value="freshest">Freshest data</option><option value="oldest">Oldest data</option><option value="area">Area</option><option value="section">Section</option></select></label></div> : <span className="nc-map-readonly-badge"><i className="fa-solid fa-lock" />Read only</span>}</div>
      {workspaceView === 'table' ? <>
