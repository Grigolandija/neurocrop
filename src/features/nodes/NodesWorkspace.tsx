...
export default function NodesWorkspace() {
  ...
  if (status === 'loading') return <div className="node-detail-empty" data-react-nodes-workspace aria-busy="true"><i className="fa-solid fa-spinner fa-spin" /><h2>Loading sensor nodes…</h2></div>
  if (status === 'error') return <div className="node-detail-empty" data-react-nodes-workspace role="alert"><h2>Nodes could not be loaded</h2><p>{error}</p><button className="button-new secondary" onClick={() => setRefreshToken((value) => value + 1)}>Try again</button></div>

  if (routeNodeId) {
    if (!selectedNode) return <div className="node-detail-page" data-react-nodes-workspace><button type="button" className="node-detail-back" onClick={() => navigate('/nodes')}><i className="fa-solid fa-chevron-left" />Nodes</button><section className="node-detail-empty"><h2>Node not found</h2><p>This node is no longer available in the current workspace.</p></section></div>
    ...
    return <div className="node-detail-page" data-react-nodes-workspace>
      <nav className="node-detail-breadcrumbs" aria-label="Breadcrumb"><button onClick={() => navigate('/nodes')}>Nodes</button><i className="fa-solid fa-chevron-right" /><span>{selectedNode.name}</span></nav>
      <header className="node-detail-head"><div><p>{selectedNode.devEui}</p><h2>{selectedNode.name}</h2><span>{selectedNode.areaName} · {selectedNode.sectionName}</span></div><div className="node-detail-actions"><button type="button" className="node-detail-secondary-action actionable" onClick={() => openEditor(selectedNode)}><i className="fa-solid fa-pen" />Edit node</button></div></header>
      ...
      <section className="node-detail-overview">
        <div className="node-detail-health"><span className="node-detail-orbit" data-tone={state.tone}><i className="fa-solid fa-microchip" /></span><div><span className="node-detail-status" data-tone={state.tone}><i className="fa-solid fa-circle" />{state.label}</span><h3>{selectedNode.transportStatus === 'offline' ? 'No recent uplink' : health.tone === 'optimal' ? 'Reporting normally' : 'Device diagnostics require review'}</h3><p>Last payload {lastPayload.relative}</p></div></div>
        <div className="node-detail-facts">
          <div><small>Battery</small><strong>{Number.isFinite(selectedNode.level) ? `${selectedNode.level}%` : '—'}</strong><span className="node-detail-track"><i style={{ width: `${Number(selectedNode.level) || 0}%` }} /></span>{Number.isFinite(selectedNode.batteryMv) ? <p>{(selectedNode.batteryMv / 1000).toFixed(2)} V</p> : null}</div>
          <div><small>Signal</small><strong>{Number.isFinite(selectedNode.rssi) ? `${selectedNode.rssi} dBm` : '—'}</strong><p>{Number.isFinite(selectedNode.snr) ? `SNR ${selectedNode.snr}${Number.isFinite(selectedNode.spreadingFactor) ? ` · SF${selectedNode.spreadingFactor}` : ''}` : 'SNR unavailable'}</p></div>
          <div><small>Reporting mode</small><strong>{getReportingModeLabel(selectedNode.profile)}</strong><p>{selectedNode.transportStatus === 'live' ? 'Live uplink' : state.label}</p></div>
        </div>
      </section>
      <div className="node-detail-columns">
        <section className="node-detail-section"><header><p>Hardware</p><h3>Installed sensors</h3></header><div className="node-detail-sensors">
          {sensorsLoading ? <p className="node-detail-muted"><i className="fa-solid fa-spinner fa-spin" /> Loading detected sensors…</p> : detected.length ? detected.map((sensor, index) => {
          ...
          }) : <p className="node-detail-muted">No sensor presence information was reported.</p>}
        </div></section>
        <section className="node-detail-section"><header><p>Diagnostics</p><h3>Latest device report</h3></header><dl className="node-detail-diagnostics">
          <div><dt>Firmware</dt><dd>{selectedNode.firmwareVersion || 'Unavailable'}</dd></div>
          <div><dt>Read failures</dt><dd>{Number.isFinite(Number(counters.read_fail)) ? counters.read_fail : 'Unavailable'}</dd></div>
          <div><dt>Transmit failures</dt><dd>{Number.isFinite(Number(counters.tx_fail)) ? counters.tx_fail : 'Unavailable'}</dd></div>
          <div><dt>Last payload</dt><dd>{lastPayload.absolute}</dd></div>
          {flags.length ? <div><dt>Fault flags</dt><dd>{flags.join(' · ')}</dd></div> : null}
          <div><dt>Signal detail</dt><dd>{formatSignal(selectedNode)}</dd></div>
        </dl></section>
      </div>
  ...
  return <div className="node-fleet-page" data-react-nodes-workspace>
    <header className="node-fleet-page-head"><div><p>Device fleet</p><h2>Sensor nodes</h2><span>Hardware availability, battery, signal, installed sensors, and latest uplink freshness.</span></div><button type="button" className="node-fleet-primary-action actionable" onClick={openRegistration}><i className="fa-solid fa-plus" />Register node</button></header>
    <section className="node-fleet-stats" aria-label="Node fleet status">
      <div><span data-tone="optimal"><i className="fa-solid fa-signal" /></span><strong>{counts.online}</strong><small>Online</small></div>
      <div><span data-tone="warning"><i className="fa-solid fa-battery-quarter" /></span><strong>{counts.lowBattery}</strong><small>Low battery</small></div>
      <div><span data-tone="critical"><i className="fa-solid fa-triangle-exclamation" /></span><strong>{counts.faults}</strong><small>Faults</small></div>
      <div><span data-tone="offline"><i className="fa-solid fa-link-slash" /></span><strong>{counts.offline}</strong><small>Offline</small></div>
    </section>
    {feedback ? <div className="management-notice success"><i className="fa-solid fa-circle-check" />{feedback}</div> : null}
    <section className="nc-node-list">
      <div className="nc-list-toolbar"><label className="nc-search-field"><i className="fa-solid fa-magnifying-glass" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, DevEUI or section" /></label><div className="nc-toolbar-selects">
        <label className="block"><span className="sr-only">Filter by area</span><select value={areaFilter} onChange={(event) => setAreaFilter(event.target.value)}><option value="all">All areas</option>{areas.map((area) => <option value={area.id} key={area.id}>{area.name}</option>)}</select></label>
        <label className="block"><span className="sr-only">Filter by state</span><select value={stateFilter} onChange={(event) => setStateFilter(event.target.value)}><option value="all">All states</option><option value="healthy">Healthy</option><option value="fault">Fault</option><option value="offline">Offline</option></select></label>
      </div></div>
      <div className="nc-data-table-wrap"><table className="nc-data-table nc-node-table"><thead><tr><th>Node</th><th>Location</th><th>State</th><th>Battery</th><th>Signal</th><th>Last payload</th><th /></tr></thead><tbody>
...
function SensorRow({ sensor, index, configurable, busy, onSave }: { sensor: Sensor; index: number; configurable: boolean; busy: boolean; onSave: (sensor: Sensor, role: string, label: string) => Promise<void> }) {
  ...
  return <div className="nc-node-sensor-row"><span><i className={`fa-solid ${index % 2 ? 'fa-temperature-half' : 'fa-wave-square'}`} /></span><div><strong>{sensorLabel(sensor)}</strong><small>Detected · {sensor.port || `Port ${index + 1}`}</small>{configurable ? <div className="nc-node-sensor-config"><select value={role} onChange={(event) => setRole(event.target.value)}>{sensorRoles.map(([value, copy]) => <option value={value} key={value}>{copy}</option>)}</select><input value={label} maxLength={80} onChange={(event) => setLabel(event.target.value)} /><button type="button" disabled={busy} onClick={() => void onSave(sensor, role, label.trim() || 'Temperature probe')}>Save purpose</button></div> : null}</div><span className="node-detail-status" data-tone="optimal"><i className="fa-solid fa-circle" />Active</span></div>
}
...
function RegistrationModal({ registration, areas, sections, busy, error, onChange, onAreaChange, onClose, onSubmit }: { registration: Registration; areas: Area[]; sections: Section[]; busy: boolean; error: string; onChange: (value: Registration) => void; onAreaChange: (id: string) => void; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  ...
  return <div className="nc-nodes-modal-layer"><button className="nc-nodes-modal-backdrop" onClick={onClose} aria-label="Close" /><section className="management-modal-shell node-edit-modal node-register-modal" role="dialog" aria-modal="true" aria-labelledby="nodeRegistrationTitle"><header className="node-edit-modal-head"><div><p className="eyebrow">Register hardware</p><h2 id="nodeRegistrationTitle">Connect a sensor node</h2><span>Assign its DevEUI to the monitored section that will receive incoming readings.</span></div><button type="button" className="node-edit-close" onClick={onClose} aria-label="Close register node dialog"><i className="fa-solid fa-xmark" /></button></header>
    {areas.length ? <form className="node-edit-form node-register-modal-form" onSubmit={onSubmit}><label className="node-edit-field"><span>Area</span><select value={registration.areaId} onChange={(event) => onAreaChange(event.target.value)}>{areas.map((area) => <option value={area.id} key={area.id}>{area.name}</option>)}</select></label><label className="node-edit-field"><span>Section</span><select value={registration.sectionId} disabled={!availableSections.length} onChange={(event) => onChange({ ...registration, sectionId: event.target.value })}>{availableSections.length ? availableSections.map((section) => <option value={section.id} key={section.id}>{section.name}</option>) : <option value="">No sections in this area</option>}</select></label><label className="node-edit-field field-wide"><span>DevEUI</span><input autoFocus value={registration.devEui} onChange={(event) => onChange({ ...registration, devEui: normalizeDevEui(event.target.value) })} placeholder="70B3D57ED006ABCD" minLength={16} maxLength={16} pattern="[0-9A-Fa-f]{16}" required /></label>{error ? <p className="management-modal-error field-wide" role="alert">{error}</p> : null}<footer className="node-edit-footer field-wide"><button type="button" className="button-new secondary" onClick={onClose}>Cancel</button><button type="submit" className="button-new primary" disabled={busy || !registration.sectionId}><i className={`fa-solid ${busy ? 'fa-spinner fa-spin' : 'fa-plus'}`} />{busy ? 'Registering…' : 'Register node'}</button></footer></form> : <div className="node-register-empty"><p>Create an Area and its first Section before registering a node.</p><button type="button" className="button-new secondary" onClick={onClose}>Close</button></div>}
  </section></div>
}
...
function NodeEditor({ editor, areas, sections, busy, error, onChange, onAreaChange, onClose, onSave, onRemove }: { editor: Editor; areas: Area[]; sections: Section[]; busy: boolean; error: string; onChange: (value: Editor) => void; onAreaChange: (id: string) => void; onClose: () => void; onSave: (event: FormEvent) => void; onRemove: () => Promise<void> }) {
  ...
  return <div className="nc-nodes-modal-layer"><button className="nc-nodes-modal-backdrop" onClick={onClose} aria-label="Close" /><section className="management-modal-shell node-edit-modal" role="dialog" aria-modal="true" aria-labelledby="nodeManagementTitle"><header className="node-edit-modal-head"><div><p className="eyebrow">Node configuration</p><h2 id="nodeManagementTitle">Edit node</h2><span>Update its identity and assignment.</span></div><button type="button" className="node-edit-close" onClick={onClose} aria-label="Close edit node dialog"><i className="fa-solid fa-xmark" /></button></header>
    <form className="node-edit-form" onSubmit={onSave}><label className="node-edit-field"><span>Node display name</span><input name="modalNodeName" autoFocus required value={editor.name} onChange={(event) => onChange({ ...editor, name: event.target.value })} /></label><label className="node-edit-field"><span>DevEUI</span><input name="modalNodeDevEui" required value={editor.devEui} minLength={16} maxLength={16} pattern="[0-9A-Fa-f]{16}" onChange={(event) => onChange({ ...editor, devEui: normalizeDevEui(event.target.value) })} /></label><label className="node-edit-field"><span>Assigned area</span><select name="modalNodeSiteId" value={editor.areaId} onChange={(event) => onAreaChange(event.target.value)}><option value="">Unassigned</option>{areas.map((area) => <option value={area.id} key={area.id}>{area.name}</option>)}</select></label><label className="node-edit-field"><span>Assigned section</span><select name="modalNodeSectionId" required={availableSections.length > 0} disabled={!availableSections.length} value={editor.sectionId} onChange={(event) => onChange({ ...editor, sectionId: event.target.value })}>{availableSections.length ? availableSections.map((section) => <option value={section.id} key={section.id}>{section.name}</option>) : <option value="">No sections available</option>}</select></label><p className="node-move-note field-wide">Moving a node keeps its identity. Future readings will belong to the selected Area and Section.</p>
      {error ? <p className="management-modal-error field-wide" role="alert">{error}</p> : null}
      <section className="node-remove-zone field-wide" aria-labelledby="remove-node-title"><div className="node-remove-head"><h3 id="remove-node-title">Remove node</h3><p>Choose what should happen to measurements already collected by this node.</p></div><fieldset className="node-delete-history-options"><legend>Measurement history</legend><label><input type="radio" checked={editor.history === 'keep'} onChange={() => onChange({ ...editor, history: 'keep' })} /><span><strong>Keep measurement history</strong><small>Retain historical data for trends and exports.</small></span></label><label data-danger><input type="radio" checked={editor.history === 'delete'} onChange={() => onChange({ ...editor, history: 'delete' })} /><span><strong>Delete measurement history</strong><small>Permanently delete every measurement collected by this node.</small></span></label></fieldset><div className="node-remove-actions"><label><input type="checkbox" checked={editor.confirmed} onChange={(event) => onChange({ ...editor, confirmed: event.target.checked })} /><span>I understand and want to remove this node</span></label><button type="button" disabled={!editor.confirmed || busy} onClick={() => void onRemove()}><i className="fa-solid fa-trash-can" />Remove node</button></div></section>
      <footer className="node-edit-footer field-wide"><button type="button" className="button-new secondary" onClick={onClose}>Cancel</button><button type="submit" className="button-new primary" disabled={busy || !editor.sectionId}><i className={`fa-solid ${busy ? 'fa-spinner fa-spin' : 'fa-check'}`} />{busy ? 'Saving…' : 'Save changes'}</button></footer>
    </form>
  </section></div>
}
