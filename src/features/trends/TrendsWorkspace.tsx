...
function normalizeExportMap(payload: JsonRecord): Omit<ExportMapState, 'status' | 'error'> {
  ...
  rawObjects.forEach((object) => {
    ...
    positions.set(devEui, { left: Math.max(1, Math.min(99, (x + objectWidth / 2) / widthM * 100)), top: Math.max(1, Math.min(99, (lengthM - y - objectHeight / 2) / lengthM * 100)) })
  })
  ...
  const objects = rawObjects.flatMap((object): ExportMapObject[] => {
    ...
    return [{
      ...
      left: x / widthM * 100,
      top: (lengthM - y - objectHeight) / lengthM * 100,
