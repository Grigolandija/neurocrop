...
export default function TrendsWorkspace() {
  ...
  return <main className="nc-trends-page" aria-busy={status === 'loading' || nodeHistoryLoading}>
    ...
    {exportOpen && selectedSection ? <ModalPortal><div className="nc-trends-export-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !exportBusy) setExportOpen(false) }}><div className="nc-trends-export-modal" role="dialog" aria-modal="true" aria-labelledby="nc-trends-export-title">
      ...
      <section className="nc-trends-export-options">
        <label><span>{tx("Scope")}</span><select value={exportScope} onChange={(event) => setExportScope(event.target.value as ExportScope)}><option value="section">{tx("Current Section")} — {selectedSection.name}</option><option value="area">{tx("Entire Area")} — {selectedSection.areaName}</option></select><small>{exportScope === 'section' ? tx("Export measurements from one Section.") : `${sections.filter((section) => section.areaId === selectedSection.areaId).length} ${tx("Sections will be included.")}`}</small></label>
        <label><span>{tx("Period")}</span><select value={exportRange} onChange={(event) => setExportRange(event.target.value as RangeKey)}>{(Object.keys(rangeConfig) as RangeKey[]).map((key) => <option value={key} key={key}>{key} — {tx(rangeConfig[key].label)}</option>)}</select><small>{tx("Times are exported in Europe/Vilnius time.")}</small></label>
      </section>
