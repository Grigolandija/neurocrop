...
function validateRegistrationInput(email: string, name: string, organizationName: string, password: string) {
...
function RegisterPage() {
  ...
  async function register(event: FormEvent<HTMLFormElement>) {
    ...
    const validationError = validateRegistrationInput(email, name, organizationName, password)
    if (validationError) {
      ...
      <div className="login-layout">
        ...
        <section className="login-form-panel" aria-labelledby="registerTitle">
          ...
          <form className="mt-8 space-y-5" onSubmit={register} noValidate>
            ...
            <label className="block"><span className="text-sm font-bold text-ink/76">Password</span><input className="login-field mt-2" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" minLength={12} required placeholder="At least 12 characters" /></label>
            {error ? <p id="registerError" className="rounded-2xl bg-[#f9e3df] px-4 py-3 text-sm font-semibold text-ember" role="alert">{error}</p> : null}
