...
assert(dashboard.includes('data-all-workspaces-mounted'), 'Authenticated workspaces must mount together after login.')
assert(dashboard.includes('allWorkspacesReady'), 'The full workspace loader must be limited to initial dashboard preparation.')
assert(dashboard.includes('workspaces.map'), 'Dashboard must keep prepared workspaces mounted for instant navigation.')
assert(dashboard.includes('hidden={workspace.route !== activeRoute}'), 'Navigation must only toggle visibility of already-mounted workspaces.')
assert(!dashboard.includes('lazy('), 'Dashboard workspaces must be bundled before the authenticated shell is shown.')
assert(dashboard.includes('prepareWorkspaceData'), 'Shared workspace data must be prepared once after authentication.')
assert(dashboard.includes('prefetchWorkspaceData'), 'Authenticated dashboard must warm shared workspace data.')
assert(dashboard.includes("host.querySelector('[aria-busy=\"true\"]')"), 'Initial dashboard preparation must wait for mounted workspaces to settle.')
