...
assert(dashboard.includes('const [visitedState, setVisitedState]'), 'Dashboard must remember workspaces the user has already opened.')
assert(dashboard.includes("mode={active ? 'visible' : 'hidden'}"), 'Inactive visited workspaces must pause effects while preserving UI state.')
assert(dashboard.includes('hidden={!active}'), 'Only the active preserved workspace may remain visible.')
