...
assert(store.includes('useWorkspaceStructureVersion'), 'Structural workspace changes must expose a focused React subscription.')
assert(sections.includes('useWorkspaceStructureVersion'), 'Sections must refresh after an Area is created without a page reload.')
assert(nodes.includes('useWorkspaceStructureVersion'), 'Nodes must refresh its Section choices without a page reload.')
