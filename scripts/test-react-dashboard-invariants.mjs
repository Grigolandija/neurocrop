...
assert(!dashboard.includes('workspaceModuleLoaders'), 'Inactive workspace modules must remain lazy instead of competing with the active route.')
assert(!dashboard.includes('prefetchWorkspaceData'), 'Dashboard must not fetch data for unopened workspaces.')
assert(!dashboard.includes('installEChartsEngine'), 'Charting must load only with Readings or Trends, not with the dashboard shell.')
