...
assert(runtime.includes('backendScoreGroups:') && runtime.includes('scoreGroups: scoreGroupsWithImpact') && runtime.includes('class="triage-score-impact-row"'), "Simple Overview must explain backend GS point deductions by agronomic domain");
...
assert(runtime.includes('data-lighting-schedule="start"') && runtime.includes('data-lighting-schedule="end"') && runtime.includes('darkThresholdLux'), "Crop profiles must expose a compact lighting schedule and darkness threshold");
...
assert(runtime.includes('function renderTriageFeedbackControls(') && runtime.includes('data-triage-feedback="completed"') && runtime.includes('${renderTriageActionHistory()}'), "Simple Overview must expose backend action feedback and verified history in the visible triage workspace");
...
assert(runtime.includes('function getActionHistoryPresentation(') && runtime.includes('rawCurrentValue !== null') && runtime.includes('rawBaselineValue !== null') && runtime.includes('insufficient_data:') && runtime.includes('Not checked: action was deferred'), "Action history must separate recorded feedback from robust before/after sensor verification and never render null as zero");
...
assert(runtime.includes('const backendPriorityAction = availableBackendActions[0] || null') && runtime.includes('globalSnapshots.find((snapshot) => snapshot.zone.id === backendPriorityAction.sectionId)') && runtime.includes('Farm priority') && runtime.includes('Selected section score') && runtime.includes('data-site-id="${escapeAttribute(prioritySnapshot.site.id)}"'), "Today must separate the farm-wide priority from the selected Section score and preserve the priority route context");
assert(runtime.includes('overview-area-card-grid') && runtime.includes('overview-selected-area-sections') && runtime.includes('data-overview-section-card') && runtime.includes("Today's farm brief") && runtime.includes('const sectionCard = event.target.closest("[data-overview-section-card]")') && styles.includes('.overview-area-card-grid') && styles.includes('overflow-x: auto'), "Today must keep many Areas in a scalable rail and expand sections only for the selected Area");
