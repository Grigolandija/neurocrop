...
assert(markup.includes('id="areasWorkspaceMount"') && dashboardPage.includes('createPortal(<AreasWorkspace />') && appSource.includes("import './styles/areas-workspace.css'"), "Areas must mount as a dedicated React workspace instead of exposing the legacy management renderer");
...
assert(markup.includes('id="sectionsWorkspaceMount"') && dashboardPage.includes('createPortal(<SectionsWorkspace />') && appSource.includes("import './styles/sections-workspace.css'"), "Sections must mount as a dedicated React workspace instead of exposing the legacy management renderer");
...
assert(runtime.includes("const metricChartColorTokens = Object.freeze") && runtime.includes("visualMap: []") && !runtime.includes("function buildTrendValueColorPieces"), "sensor charts must retain metric identity colors instead of borrowing semantic warning and danger colors");
...
assert(runtime.includes("function renderAlertsManagementPage(globalSnapshots = [])") && runtime.includes('data-alerts-page-filter=') && runtime.includes('data-alerts-view-context=') && runtime.includes('data-alerts-acknowledge=') && runtime.includes('data-alerts-snooze=') && runtime.includes('data-alerts-resolve='), "Alerts must retain the redesign information architecture with working filters, context and workflow actions");
assert(runtime.includes("function acknowledgeAlertsPageItems(items)") && runtime.includes("function updateAlertsPageItemWorkflow(item, action)") && runtime.includes("backendAlertRecords") && runtime.includes('window.NeuroCropApi.getAlerts("all")') && runtime.includes("window.NeuroCropApi.acknowledgeAlert(item.id, { context: item })") && runtime.includes("window.NeuroCropApi.snoozeAlert(item.id") && runtime.includes("window.NeuroCropApi.resolveAlert(item.id") && apiFacade.includes("acknowledgeAlert:") && apiFacade.includes("createIntervention:"), "Alert review and intervention state must use persistent organization-scoped backend workflows");
...
assert(runtime.includes('getTrendSeriesColor(index, item.option.key)') && runtime.includes('color: comparisonChartColors'), "Trend and comparison charts must use the dedicated measurement palette");
...
assert(runtime.includes('fontFamily: "IBM Plex Sans, sans-serif"') && !runtime.includes('fontFamily: "Manrope, sans-serif"') && !runtime.match(/fontWeight: (?:7|8|9)\d\d/), "Charts must follow the new typography system without legacy font families or excessive weights");
