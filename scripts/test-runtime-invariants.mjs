...
assert(apiFacade.includes('getActionHistory:') && runtime.includes('today-priority-history') && runtime.includes('Sensor result appears after the next reading'), "Recent action history must explain when sensor verification becomes available");
...
assert(runtime.includes('function getActionHistoryPresentation(') && runtime.includes('rawCurrentValue !== null') && runtime.includes('Not checked: action was deferred'), "Action history must separate recorded feedback from sensor verification and never render null as zero");
...
assert(runtime.includes('isSiteView ? siteZoneIds.has(action.sectionId) : action.sectionId === zone.id') && runtime.includes('const backendPriorityAction = scopedBackendActions[0] || null') && runtime.includes('Selected section priority'), "Zone score and priority cards must never display an action from another section");
assert(runtime.includes('overview-area-card-grid') && runtime.includes('overview-selected-area-sections') && runtime.includes('data-overview-section-card') && runtime.includes('Farm at a glance') && runtime.includes('const sectionCard = event.target.closest("[data-overview-section-card]")') && styles.includes('.overview-area-card-grid') && styles.includes('overflow-x: auto'), "Overview must keep many Areas in a scalable rail and expand sections only for the selected Area");
