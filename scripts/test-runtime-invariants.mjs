...
assert(runtime.includes('const limitingFactorActivation = smoothScoreProgress((worstGroup.severity - 0.25) / 0.75);'), "frontend fallback limiting-factor pressure must activate only for material stress");
assert(runtime.includes('source: "frontend-fallback"') && runtime.includes('scoreModelVersion: "2.0.0"'), "frontend fallback scores must identify the same explicit model version as the backend");
