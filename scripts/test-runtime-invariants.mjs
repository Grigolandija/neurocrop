...
assert(runtime.includes("function renderAlertsManagementPage(globalSnapshots = [])") && runtime.includes('data-alerts-page-filter=') && runtime.includes('data-alerts-view-context=') && runtime.includes('data-alerts-acknowledge='), "Alerts must retain the redesign information architecture with working filters, context and acknowledgement actions");
assert(runtime.includes("function acknowledgeAlertsPageItems(items)") && runtime.includes("reviewedAlertsStorageKey") && apiFacade.includes("acknowledgeAlert:"), "Alert review state must persist locally until the backend alert endpoint becomes available through the API facade");
